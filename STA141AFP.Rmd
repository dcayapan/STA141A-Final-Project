...
plot(merged_nba_clean$Salary_sqrt, residuals_sqrt,
     ...
     xlab = "Inverse of Individual Salaries",
     ylab = "Residuals")
abline(h = 0, col = "red")  # Add a horizontal line at y = 0 for reference
```
...
ggplot(merged_nba_clean, aes(x = Total_sqrt, y = WP)) +
  ...
  labs(title = "Sqaure Root Transformation of Team spending vs. Win Percentage",
       x = "Exponential of Team Spending",
       y = "Win Percentage") +
  theme_minimal()
...
plot(merged_nba_clean$Total_sqrt, residuals_sqrt1,
     ...
     xlab = "Inverse of Total Spending",
     ylab = "Residuals")
abline(h = 0, col = "red")  # Add a horizontal line at y = 0 for reference
```
...
```
Conclusion: Based on the several questions that we generated, we have concluded that NBA Players' salaries are justified. With high correlation of 0.90 + on all 3 questions and clustering we were able to gather that NBA players with similar salaries are paid well based on their individual performances and their affect on team win percentages.
